...
@section('content_header')
    <h1><b>Listado de Horarios creados</b></h1>
    <hr>
@stop
            ...
            <div class="card card-outline card-primary">
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        <a href="{{url('/admin/horarios/create')}}" class="btn btn-primary">Crear nuevo</a>
                    </div>
                    <!--/.card-tools -->
                </div>
                <!--/.card-header--> 
                    ...
                    <table id="example1" class="table table-bordered table-hover table-striped table-sm">
                        ...
                        @foreach($horarios as $horario)
                            <tr>
                                ...
                                <td style="text-align: center">{{$horario->grupoAcademico->semestre->nombre}}</td>
                                <td style="text-align: center">{{$horario->grupoAcademico->paralelo}}</td>
                                <td style="text-align: center">{{$horario->grupoAcademico->cant_estudiantes}}</td>
                                ...
                                <td style="text-align: center">
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        <a href="{{url('/admin/horarios/'.$horario->id.'/edit')}}" style="border-radius: 0px 0px 0px 0px" class="btn btn-success btn-sm"><i class="fas fa-pencil-alt"></i></a>
                                        <form action="{{url('/admin/horarios', $horario->id)}}" method="post"
                                                onclick="preguntar{{$horario->id}}(event)" id="miformulario{{$horario->id}}">
                                            @csrf 
                                            @method('DELETE')
                                            ...
                                            function preguntar{{$horario->id}}(event){
                                                ...
                                                Swal.fire({
                                                    ...
                                                    if (result.isConfirmed){
                                                        var form=$('#miformulario{{$horario->id}}');
                                                        form.submit();
                                                    }
                                                });
                                            }
